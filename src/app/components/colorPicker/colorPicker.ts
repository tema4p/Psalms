...
export class ColorPicker {
  ...
  initPalette(){
    ...
    let pixelRatio = this.getPixelRatio(this.ctxPalette);

    console.log(pixelRatio);

    let width = currentWidth * 90/100;
  ...
  initChooser(){
    ...
    let ctx = canvasChooser.getContext("2d");
    ...
    let eventChangeColorChooser = (event) => {
  ...
  getColor(event, canvas, context, fromChooser : boolean) : string {
    ...
    let x = (touchX - bounding.left) * this.getPixelRatio(context);
    let y = (touchY - bounding.top) * this.getPixelRatio(context);
